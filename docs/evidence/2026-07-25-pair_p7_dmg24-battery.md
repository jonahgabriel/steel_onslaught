...
- **Uncontrolled range-30 standoff (`harpoon_gun`).** This arm's blue loadout closes only one of blue's two long-range weapons: `artillery_mortar` was cut to range 30 (the approach-fix lever), but `harpoon_gun` stayed at range 30, uncapped, through this arm and every other arm in the P1–P7 sweep. Blue therefore retained an ~18–22 cell standoff advantage against red's longest weapon (`machine_gun`, range 12) that this arm's design did not control for and this document did not previously disclose. Found post hoc during the SO-RANGECAP design pass (PR #197); does not change any figure reported above. See OMN-15119.
