...
**Does NOT replicate closely — diverges materially:**
- **The abort rate.** 63.3% (original) vs 23.3% (replication) — roughly a 2.7× difference in the fraction of matches that abort before reaching a decidable terminal. `play_terminal_fraction` moved from 0.3667 to 0.7667. The replication still fails both trust gates (0.7667 < 0.90 and < 0.95), so the qualitative verdict ("this arm has a real trust-gate problem") does not flip — but the magnitude swings enough that the original 63% figure is **not** a stable, reproducible constant of this arm. It sits somewhere in a range that includes at least [23%, 63%] on two independent same-day n=30 samples, same wiring, same overlay, same model pin (`glm-4.5`).

### 5.3 Interpretation

This is a genuine update to how PR #154's confound caveat should be read, in one specific direction: the **abort-rate magnitude** in that doc (63%) should not be treated as a fixed characteristic of `glm-4.5` on this contract — it is closer to a noisy upper bound observed on one sample than a stable rate. Plausible contributors (not established by this data, offered as hypotheses only): time-of-day load variance on the shared z.ai CODING pool, or ordinary sampling variance in a per-turn semantic-validity rate multiplied across ~5–6 rounds per match (a moderate per-turn improvement compounds quickly over several rounds into a large terminal-rate swing). Distinguishing these would need more than two batteries.

What does **not** need updating: the core findings PR #154 and the session handoff draw from this arm — red never wins, red suppresses utility far below chance, blue does *not* suppress utility (unlike the other three lineages) — all survive this replication intact, and in blue's case, more strongly confirmed than before. The confound caveat itself (already present in the merged doc: "committed red plans are a biased survivor sample… directional only") remains the correct level of hedging for the capability-axis reading; if anything, this replication is a reason to hedge the *abort-rate number* harder while leaving the *win-rate and keep-rate* claims as they stand.
