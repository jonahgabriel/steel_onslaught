...
**Model:** live Qwen3.6-35B-A3B, both seats, n=30, seeds 5001–5030, single-invocation-per-seed battery (no resume/segmentation — see §0), `all_replay_valid=true` (1/1 both players, all 30 matches).
...
## 0. Raw-file provenance (single-invocation battery, no resume)
...
- **Utility keep-rate per seat** = utility cards programmed into a register (`plan_committed.registers[].card_id` matching `card.utility.%`) ÷ utility cards dealt (`hand_dealt.card_ids` matching `card.utility.%`), same method as the P1/P2/P3 docs. Filtered to the 30 valid `match_id`s (there are no orphan/crashed matches to exclude in this battery — see §0).
...
The point estimate continues to climb monotonically across all four points and remains inside the DIRECTIONAL band, short of the 0.35 COMPETITIVE threshold. **The per-unit-dose marginal increment, however, does not continue to accelerate on this step:**
...
The marginal rate peaked at the ×3→×5.5 step (5.52 pp/unit) and **fell** at the ×5.5→×8 step (4.14 pp/unit) — the opposite of the "monotonic and accelerating" framing that described the first three points alone. This does not establish that the curve is now decelerating in any calibrated sense either: **four points, n=30 per arm, is not enough to fit a dose-response curve of any functional form** — it is enough to say the direction is still positive but the naive "keep raising the dose and the rate keeps accelerating" read from three points does not hold up against the fourth. No calibrated "predicted competitive multiplier" can be extrapolated from this data, and none is offered here.
